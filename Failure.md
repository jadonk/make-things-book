# Why Does Technology Fail?

An introduction to Maker ethics

## A Diagnosis of Failure

Catastrophic failures have often exhibited a similar set if causes.

These same causes can be observed in failures with somewhat less obvious harmful effects.


### Drift into failure

Sources:
* https://sidneydekker.com/drift-into-failure


Certain pressures on technological systems are sure to exist


## The Ethics of Failure

